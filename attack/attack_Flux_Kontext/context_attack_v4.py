...
def compute_losses(cond_latents_adv, cond_latents_ref,
    ...
    # attention map loss
    # 对齐 softmax(QK^T / sqrt(d)) 之后的注意力分布
    La = 0.0
    n_attn = 0
...
def main():
    ...
    for sample in dataset:
        ...
        for step in tqdm(range(args.attack_steps)):
            ...
            embeds = prompt_embeds_pool[prompt_idx] # torch.Size([1, 512, 4096])
            pooled = pooled_prompt_embeds_pool[prompt_idx] # torch.Size([1, 768])
            text_ids = text_ids_pool[prompt_idx]

            noise = torch.randn((1,16,64,64), device=device, dtype=mp_dtype) # torch.Size([1, 16, 64, 64])
            # 将当前对抗图像编码为 context latent
            cond_latents_adv = vae.encode(adv).latent_dist.mode() # torch.Size([1, 16, 64, 64])
            cond_latents_adv = (cond_latents_adv - vae_shift) * vae_scale

            # 将随机噪声视为 target noisy latent（与原始 DeContext 一致）
            noisy_latents = noise # torch.Size([1, 16, 64, 64])

            # pack target latent
            packed_noisy_latents = FluxKontextPipeline._pack_latents( # torch.Size([1, 1024, 64])
                noisy_latents,
                bsz,
            ...
            packed_cond_adv = FluxKontextPipeline._pack_latents( # torch.Size([1, 1024, 64])
                cond_latents_adv,
                bsz,
            ...
            packed_input_adv = torch.cat([packed_noisy_latents, packed_cond_adv], dim=1) # torch.Size([1, 2048, 64])
            packed_input_ref = torch.cat([packed_noisy_latents, packed_cond_ref], dim=1) # torch.Size([1, 2048, 64])

            # 采样高噪声区间的 timestep（与原始 DeContext 一致）
            bsz = 1
            timesteps = torch.randint( # tensor([983], device='cuda:2')
                980,
            ...
            timesteps_expanded = timesteps.expand(bsz).to(device=device, dtype=mp_dtype) / float(scheduler.config.num_train_timesteps) # tensor([0.9844], device='cuda:2', dtype=torch.bfloat16)
            ...
            t_pred = time.time()
            pred = transformer( # torch.Size([1, 2048, 64])
            ...
            adv_qkv = {k: v.clone() for k, v in hook.qkv.items()} # 共26层
            ...
            with torch.no_grad():
                ...
                t_ref = time.time()
                ref_pred = transformer( # torch.Size([1, 2048, 64])
