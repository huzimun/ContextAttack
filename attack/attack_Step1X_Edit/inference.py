...
from modules.attention import set_current_timestep, plot_context_proportion
import numpy as np
import torch
...
class ImageGenerator:
    ...
    def denoise(
        ...
        if show_progress:
            ...
            pbar = itertools.pairwise(timesteps)
        for idx, (t_curr, t_prev) in enumerate(pbar):
            set_current_timestep(t_curr, timestep_idx=idx)
            if img.shape[0] == 1 and cfg_guidance != -1:
                img = torch.cat([img, img], dim=0)
            t_vec = torch.full(
                (img.shape[0],), t_curr, dtype=img.dtype, device=img.device
        ...
        # 推理结束后绘图
        if idx == len(timesteps) - 2:  # 最后一个timestep
            plot_context_proportion()
...
def main():
    ...
    parser.add_argument('--prompt', type=str, default=None, help='Direct prompt (overrides json)')
    parser.add_argument('--image', type=str, default=None, help='Direct image path (overrides json)')
    ...
    if args.prompt and args.image:
        image_and_prompts = {os.path.basename(args.image): args.prompt}
    else:
        image_and_prompts = json.load(open(args.json_path, 'r'))
    ...
    for image_name, prompt in image_and_prompts.items():
        ...
        if os.getenv("TORCHELASTIC_RUN_ID") is None or dist.get_rank() == 0:
            if len(time_list) > 1:
                ...
                print(f'time for {args.output_dir}: ', time_list[0])
